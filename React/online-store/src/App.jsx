...
function App() {
  ...
  useEffect(() => {
    ...
    if (currentPriceArr && currentPriceArr.length === 2 && currentPriceArr.every((el) => !el.match(/\D/gm))) {
      const currentPrice = currentPriceArr.map((el) => parseInt(el));
      if (currentPrice[0] <= currentPrice[1]) {
        setMinMaxPrice(currentPrice);
        setValidSearchParams((prevSearchParams) => ({ ...prevSearchParams, price: currentPrice }));
      } else {
        setMinMaxPrice([]);
    ...
    if (currentScreen && currentScreen.split(" ").some((el) => screen.find((elem) => elem === el))) {
  ...
  return (
    <div className={styles.wrapper}>
      ...
      <div className={styles.filters}>
        <Filters prices={prices} memory={memory} screen={screen} searchParams={validSearchParams} setSearchParams={setSearchParams} />
      </div>
    </div>
  );
}
